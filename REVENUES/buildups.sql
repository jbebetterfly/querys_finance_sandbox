...
--- LAST UPDATE: MARTES 11 DE JUNIO 2024 (2024-09-30)
WITH 
...
FROM `btf-source-of-truth.cubo.ingresos_operaciones`
  ...
  UNION ALL SELECT * FROM `btf-finance-sandbox.Revenue.temp-fix_ingresos-ops` -- parches para diferencias con facturas de holdings y otros
),
holdings_new as (
  SELECT DISTINCT
  tax_id_client,
...
bucket_width => INTERVAL 1 MONTH,
 partitioning_columns => ['revenue_stream','service_country', 'holding_name', 'holding_cohort', 'client_segment', 'local_currency', 'sponsor'],
value_columns => [
    ('value_lc', 'null'),
...
SELECT
  ...
  COALESCE(LAG(members) OVER(PARTITION BY holding_name,revenue_stream,service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)), 0) AS members_bop,
  LEAD(members) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)) AS next_period_members,
  COALESCE(LAG(value_lc) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)),0) AS value_lc_bop,
  LEAD(value_lc) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)) AS next_period_value_lc,
  COALESCE(LAG(value_usd_bdg) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)),0) AS value_usd_bop,
  LEAD(value_usd_bdg) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)) AS next_period_value_usd,
  COALESCE(LAG(members,2) OVER(PARTITION BY holding_name,revenue_stream,service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)), 0) AS members_bop2,
  COALESCE(LAG(value_lc,2) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)),0) AS value_lc_bop2,
  COALESCE(LAG(value_usd_bdg,2) OVER(PARTITION BY holding_name,revenue_stream, service_country,sponsor ORDER BY EXTRACT(YEAR FROM full_date), EXTRACT(MONTH FROM full_date)),0) AS value_usd_bop2,
