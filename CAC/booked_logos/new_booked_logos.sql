...
SELECT *,
CASE
WHEN LAG(hs_company_id) OVER(PARTITION BY hs_company_id ORDER BY closed_date_year, closed_date_month, hs_company_id) = hs_company_id THEN 'repetido' 
ELSE 'no repetido'
...
SELECT *,
CASE 
WHEN hs_company_repeat_check = 'repetido' AND company_count = 1 THEN 0
WHEN hs_company_repeat_check = 'no repetido' AND company_count = 1 THEN 1
ELSE NULL
END AS company_count_post_check

FROM check_company_repetido
